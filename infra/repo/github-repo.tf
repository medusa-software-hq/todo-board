...
resource "github_repository" "this" {
  name        = module.config.gh_repo_name
  description = "Variant: ${module.config.project_variant}"
...
resource "github_repository_ruleset" "default_branch" {
  ...
  rules {
    ...
    required_status_checks {
      ...
      required_check {
        context        = "${local.job_names.config} / Check config"
        integration_id = local.gh_actions_integration_id
      }

      required_check {
        context        = "${local.job_names.root_infra} / Check Terraform"
        integration_id = local.gh_actions_integration_id
      }
...
resource "github_repository_environment_deployment_policy" "production_trunk" {
  ...
  branch_pattern = default_branch_name
}
...
resource "github_repository_environment_deployment_policy" "staging_trunk" {
  ...
  branch_pattern = module.config.gh_default_branch_name
}
